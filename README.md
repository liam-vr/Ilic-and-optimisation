...
\displaylines{k^y_y =- D^2 \frac{I_0}{mc} \left[Q^{R'}_{\text{pr,2'}} - Q^{L'}_{\text{pr,2'}} \right](0, \lambda') \left\{1 - \exp\left[-\frac{1}{2 \bar{w}^2} \right] \right\}, \\
    k^y_\phi =-D^2 \frac{I_0}{mc} \left[ \frac{\partial Q^{R'}_{\text{pr,2'}}}{\partial \delta'} + \frac{\partial Q^{L'}_{\text{pr,2'}}}{\partial \delta'} \right](0, \lambda') \frac{w}{2} \sqrt{\frac{\pi}{2}} \text{erf}\left[\frac{1}{\bar{w} \sqrt{2}}  \right], \\
    ...
    \mu^y_\phi =D^2 \frac{I_0}{mc} \frac{1}{c} \left(2 \left[Q^{R'}_{\text{pr,2'}} - Q^{L'}_{\text{pr,2'}} \right] - D \left[ \frac{\partial  Q^{R'}_{\text{pr,2'}}}{\partial \bar{\nu}'} - \frac{\partial  Q^{L'}_{\text{pr,2'}}}{\partial \bar{\nu}'} \right]\right) (0, \lambda') \frac{w^2}{4} \left(1 - \exp\left[-\frac{1}{2\bar{w}^2} \right] \right),}
```
and 
```math
\displaylines{k^{\phi}_y =D^2 \frac{I_0}{mc} \frac{1}{\Gamma L^2} \left[ Q^{R'}_{\text{pr,1'}} + Q^{L'}_{\text{pr,1'}} \right](0, \lambda') \left\{ \frac{w}{2} \sqrt{\frac{\pi}{2}} \text{erf} \left[\frac{1}{\bar{w} \sqrt{2}} \right] - \frac{L}{2}  \exp \left[-\frac{1}{2\bar{w}^2} \right] \right\}, \\
    k^{\phi}_\phi =D^2 \frac{I_0}{mc} \frac{1}{\Gamma L^2} \left( \frac{\partial Q^{R'}_{\text{pr,1'}}}{\partial \delta'} - \frac{\partial Q^{L'}_{\text{pr,1'}}}{\partial \delta'} -\left[ Q^{R'}_{\text{pr,2'}} - Q^{L'}_{\text{pr,2'}} \right] \right) (0, \lambda') \frac{w^2}{4} \left(1 - \exp\left[-\frac{1}{2\bar{w}^2} \right]  \right), \\
    \mu^{\phi}_y= D^2 \frac{I_0}{mc} \frac{1}{\Gamma L^2} \frac{1}{c} \frac{D+1}{D(\gamma+1)} \left( \frac{\partial Q^{R'}_{\text{pr,1'}}}{\partial \delta'} - \frac{\partial Q^{L'}_{\text{pr,1'}}}{\partial \delta'} -\left[ Q^{R'}_{\text{pr,2'}} - Q^{L'}_{\text{pr,2'}} \right] \right) (0, \lambda') \frac{w^2}{4} \left(1 - \exp\left[-\frac{1}{2\bar{w}^2} \right]  \right),}
```
```math
...
- `!M_non-linear.py`: this is the main method. The full non-linear equations of motion are applied in frame $\mathcal{M}$, which is continually updated by converting between $\mathcal{M}$ and $\mathcal{L}$ as necessary.
